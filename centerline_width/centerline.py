...
def saveCenterlineCSV(river_object=None, save_to_csv=None):
	# Save Centerline Coordinates generated by Voronoi Diagram to CSV
	centerline_width.errorHandlingSaveCenterlineCSV(river_object=river_object, save_to_csv=save_to_csv)

	with open(save_to_csv, "w") as csv_file_output:
		writer = csv.writer(csv_file_output)
		writer.writerow(["Centerline Latitude (Deg)", "Centerline Longitude (Deg)"])
		for latitude_longitude in river_object.centerlineLatitudeLongtiude:
