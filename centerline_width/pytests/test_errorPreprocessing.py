...
def test_convertColumnsToCSV_textFileRequired():
    with pytest.raises(ValueError,
                       match=re.escape('[text_file]: Requires text file')):
...
def test_convertColumnsToCSV_textFileInvalidTypes(invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
                f"[text_file]: Must be a str, current type = '{error_output}'")
    ):
...
def test_convertColumnsToCSV_textFileInvalidExtensions():
    with pytest.raises(
            ValueError,
            match=re.escape(
                "[text_file]: Extension must be a .txt file, current extension = 'csv'"
            )):
