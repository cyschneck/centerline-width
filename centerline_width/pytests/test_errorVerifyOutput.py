...
def test_riverWidthFromCenterline_transectSlopeAverage():
	...
	for i in range(0, len(test_river.centerlineEvenlySpaced), span_distance):
		if i == 0:
			groups_of_n_points.append(test_river.centerlineEvenlySpaced[0:span_distance])
		else:
			groups_of_n_points.append(test_river.centerlineEvenlySpaced[i-1:i+span_distance])
	...
	river_width_dict = test_river.riverWidthFromCenterline(transect_slope="Average",
...
def test_riverWidthFromCenterline_transectSlopeDirect():
	...
	river_width_dict = test_river.riverWidthFromCenterline(transect_slope="Direct",
															transect_span_distance=span_distance,
															apply_smoothing=False)
	# Verify same keys are used
	assert list(river_width_dict.keys()) == centerline_slope_expected
