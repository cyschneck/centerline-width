...
def test_riverWidthFromCenterline_riverObjectInvalidTypes(
...
def test_riverWidthFromCenterline_transectSpanDistanceInvalidTypes(
        invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
...
@pytest.mark.parametrize("invalid_input, error_output",
                         invalid_non_bool_options)
def test_riverWidthFromCenterline_removeIntersectionsInvalidTypes(
        invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
...
@pytest.mark.parametrize("invalid_input, error_output",
                         invalid_non_str_options)
def test_riverWidthFromCenterline_saveToCSVInvalidTypes(
        invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
        ...
        centerline_width.riverWidthFromCenterline(
            river_object=river_class_example, save_to_csv=invalid_input)
...
def test_riverWidthFromCenterline_csvInvalidExtension():
    with pytest.raises(
        ...
        centerline_width.riverWidthFromCenterline(
            river_object=river_class_example, save_to_csv="filename.txt")
...
@pytest.mark.parametrize("invalid_input, error_output",
                         invalid_non_str_options)
def test_riverWidthFromCenterline_coordinateUnitInvalidTypes(
        invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
...
@pytest.mark.parametrize("invalid_input, error_output",
                         invalid_non_str_options)
def test_riverWidthFromCenterline_coordinateReferenceInvalidTypes(
        invalid_input, error_output):
    with pytest.raises(
            ValueError,
            match=re.escape(
